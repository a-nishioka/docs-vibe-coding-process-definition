...
このプロセスは、コードを伴うフェーズの「やり方（HOW）」を補強するために Serena MCP（LSP ベースのセマンティックなコード操作を提供する MCP サーバ）を併用する。プロセスの「いつ・何を・どのゲートで（WHAT/WHEN）」は本ドキュメントが所有し続け、Serena はその中で使うツール層とする。Serena は **コードを伴う作業でのみ起動**し、ドキュメントのみの作業では使わない。

### フェーズ対応表

| フェーズ | Serena の役割 | 主なツール | 境界条件 |
|---|---|---|---|
| フェーズ2 設計（Step 1 既存コード調査） | コード構造・影響範囲をセマンティックに把握（grep より正確） | `get_symbols_overview` / `find_symbol` / `find_referencing_symbols` | 調査の手段に限定。設計判断・記録は従来通り `tasks/T-XXX_<機能名>.md` と `docs/ARCHITECTURE.md` |
| フェーズ3 実装 | シンボル単位の正確な編集を補助 | `find_symbol` / `replace_symbol_body` / `insert_after_symbol` / `insert_before_symbol` | 編集の主役は既存フロー（Claude Code の編集＋`test-driven-development`）。Serena 編集は誤りを減らせる場面で併用。コミット規律は本プロセス優先 |
| フェーズ4 テスト（Step 2 リグレッション） | 変更シンボルの参照元を洗い出し影響範囲を確定 | `find_referencing_symbols` | リグレッション判断・記録は従来通り。発見した影響は本プロセスの手戻り表に従う |

### メモリ担当表（クリーンな層分け）

Serena は独自のメモリ機能（`.serena/memories/`）と onboarding を持つが、これらは枠組みドキュメントと**二重化させない**。各知識の正典は以下で固定する。

| 知識の種類 | 正典（Source of Truth） | Serena memory |
|---|---|---|
| プロセス・フェーズ・承認ゲート | `CLAUDE.md` / `docs/PROCESS.md` | 持たない |
| アーキテクチャ・技術方針 | `docs/ARCHITECTURE.md` | 持たない |
| 技術的意思決定（ADR） | `docs/DECISIONS.md` | 持たない |
| コーディング規約 | `docs/CODING_STANDARDS.md` | 持たない |
| 要件・設計・テスト・フィードバック・マージ記録 | `tasks/T-XXX_<機能名>.md` | 持たない |
| ロードマップ・タスク候補 | `docs/OVERALL_PLAN.md` / `docs/DRAFT_PLAN.md` | 持たない |
| コード構造・シンボルマップ・参照関係 | コード自体（LSP が動的取得） | ✅ 機械的キャッシュとして可 |
| ビルド/テスト/実行コマンド | 記載があれば `docs/CODING_STANDARDS.md` 等が正典 | ✅ キャッシュ可（正典があればそちら優先） |

> ルール: Serena が memory に書いてよいのは「コードから機械的に再生成できる事実」のみ。判断・方針・仕様は書かない。重複したら枠組みドキュメントが優先。

### 導入しないもの（意図的な除外）

- **コードを伴わないフェーズ（フェーズ1/5/6/7 等）**: Serena は起動しない。要件定義・フィードバック・マージ・次プロセス判断はドキュメント中心の作業のため。
- **Serena の onboarding/memory にプロセス・設計知識を持たせない**: 上のメモリ担当表に従う。

### Serena と本プロセスの優先順位

- Serena MCP は「やり方（HOW）」を補強するツール層であり、「いつ・何を・どのゲートで（WHAT/WHEN）」は本ドキュメントが所有する。Serena の固定挙動と本ドキュメントが食い違う場合は、上表の「境界条件」に従って本ドキュメントを優先する。
- 接続方法・セットアップ手順は `README.md`「Serena MCP のセットアップ」を参照する。

---

## レビュー工程の併用

このプロセスは、各フェーズの成果物の品質を担保するために、サブエージェントによる独立レビューを併用する。プロセスの「いつ・何を・どのゲートで（WHAT/WHEN）」は本ドキュメントが所有し続け、レビューはその中で成果物を点検する HOW 層とする。レビューは「新しい成果物＋下流の承認ゲート」がある所にのみ置き、無い所（マージ後など）は検証（verification）に留める。

### 機構（共通の取り決め）

- **ダブルチェック構造**: サブエージェント独立レビュー（1次）→ ユーザー確認（2次）。1次レビューの指摘は、既存の承認ゲートに「検査済みの判断材料」として渡る。レビュー専用の新しい承認ゲートは作らない。
- **レビューのスタンス（敵対的／red-team）**: 独立レビューは確認型ではなく敵対的スタンスを既定とする。「成果物には欠陥がある」と仮定し、前提の破綻・エッジケース・異常系・抜け穴・「DoD の字面は満たすが趣旨は未達」を能動的に探す。軽いフェーズでも姿勢は同じで、深さ（時間・観点数）だけ成果物に比例させる。`receiving-code-review` と対になり、出し手が攻め、受け手は盲従も全否定もせず技術的に吟味する。
- **指摘の扱い（既存FB分類を流用）**: レビュー指摘は既存のフィードバック分類（🔴即修正／🔵テスト漏れ／🟡次サイクル／⚪対応不要／❌見送り）で振り分ける。🔴はユーザーゲート前に解消、🟡は `docs/DRAFT_PLAN.md` へ、⚪は理由を添えてスキップ。レビュー専用の新しい重要度語彙は作らない。攻めの姿勢で出た指摘もこの分類で取捨選択し、スコープ外要件を発明しない。
- **起動前提（記入 → レビュー → ゲート）**: 各フェーズの生産結果を、まず成果物（`tasks/T-XXX_<機能名>.md` のタスク文書、または作業ツリーのコード）に記入してからレビューする。レビュー対象は「書かれているが未コミット」の状態でよく、コミットのタイミングは現行どおり承認後で変えない。
- **軽重とフォールバック**: レビューの深さは成果物に比例させる。コードを伴う成果物は本格レビュー、ドキュメントのみの成果物は観点チェックリストに沿った軽量レビュー。成果物が些末な場合はセルフレビューにフォールバックしてよい。
- **優先順位**: スキル（`requesting-code-review` 等）の固定フローと本プロセスが食い違う場合は、本プロセス（`CLAUDE.md` / `docs/PROCESS.md`）を優先する。

### フェーズ対応表

| フェーズ | レビュー対象（成果物） | 主な観点 | 併用スキル | 軽重 | 出力先 |
|---|---|---|---|---|---|
| 1 要件定義 | 要件（目的/スコープ/DoD） | DoD がテスト可能か・スコープ内外の明確さ・`OVERALL_PLAN` 整合・抜け漏れ | — | 軽 | フェーズ1 承認ゲート |
| 2 設計 | 設計メモ/Todo/テスト設計 | `ARCHITECTURE` 整合・リスク網羅・テスト設計が DoD 全網羅・影響範囲 | Serena（影響範囲） | 中 | フェーズ2 承認ゲート |
| 3+4 実装・テスト | コード＋テスト結果 | 正確性バグ・規約準拠・スコープ遵守・DoD 網羅・リグレッション | requesting-code-review / receiving-code-review / verification-before-completion | 重 | フェーズ5 入力 |
| 5 フィードバック | 全ドキュメント整合・最終判断 | 各ドキュメント相互整合・FB 分類の妥当性 | — | 中 | フェーズ5 承認ゲート |
| 6 マージ | マージ前状態／マージ後状態 | 前: ドキュメント更新漏れ・クリーン状態／後: main グリーン・コンフリクト痕・記録の正しさ（＝検証） | verification-before-completion | 軽 | フェーズ6 承認ゲート |
| 7 次プロセス判断 | 昇格判断 | 昇格候補の妥当性・`OVERALL_PLAN` 整合 | brainstorming（既存） | 軽 | フェーズ7 承認ゲート |

> 補足:
> - すべての独立レビューは敵対的スタンス（red-team）を既定とする。軽重は「深さ」の違いであって「姿勢」の違いではない。
> - フェーズ3は単独レビューを持たず、フェーズ4出口でフェーズ3+4を一括レビューする。「全フェーズにレビュー」はサイクル単位で満たし、内側の実装↔テスト反復はゲート無しのまま回す。
> - フェーズ5のレビューはドキュメント整合に限定し、コードは再レビューしない（フェーズ4出口で完了済み）。
> - フェーズ6のマージ後はレビューではなく検証（verification）。独立レビューは置かない。

### 導入しないもの（意図的な除外）

- 内側の実装↔テスト反復（フェーズ3↔4）へのレビューゲート挿入（出口で一括のみ）。
- マージ後の独立レビュー（軽量検証に留める）。
- フェーズ5でのコード再レビュー（フェーズ4出口で完了）。
- レビュー専用の新しい重要度語彙（FB 分類を流用）。
- 些末なドキュメント変更へのサブエージェントレビュー強制（セルフレビュー可）。
- 敵対的レビューを揚げ足取り・スコープ拡大にすること（指摘は FB 分類で振り分け、スコープ外要件を発明しない）。

### レビューと本プロセスの優先順位

- レビューは「やり方（HOW）」を補強する層であり、「いつ・何を・どのゲートで（WHAT/WHEN）」は本ドキュメントが所有する。レビュー系スキルの固定挙動と本ドキュメントが食い違う場合は、上記「機構」「導入しないもの」に従って本ドキュメントを優先する。
...
**Step 5: 要件定義サマリーの提示**
- 提示の前に、記入済みの「## 要件」を観点チェックする（DoD がテスト可能か・スコープ内外の明確さ・`docs/OVERALL_PLAN.md` との整合・抜け漏れ）。「レビュー工程の併用」に従い、軽量レビューでよい。
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📋 要件定義サマリー
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
**Step 3: tasks/T-XXX_<機能名>.md への設計ドラフト記入（承認前）**
- レビュー可能にするため、設計の生産結果を承認前に成果物へ書き込む（「レビュー工程の併用」の起動前提に従う。記入は承認前、コミットは承認後）。
...
**Step 4: サブエージェント設計レビュー（敵対的スタンス）**
- Step 3 で記入した設計ドラフトを対象に、サブエージェントによる独立レビューを敵対的スタンスで行う（「レビュー工程の併用」に従う）。
- 攻める観点: 前提の破綻・失敗モード・破壊的変更の見落とし・`docs/ARCHITECTURE.md` との不整合・テスト設計の穴（DoD を全網羅しているか）・影響範囲の見落とし（Serena の `find_referencing_symbols` 等を併用）。
- 返ってきた指摘は既存FB分類（🔴/🔵/🟡/⚪）で振り分け、🔴はドラフトに反映してから次へ進む。🟡は `docs/DRAFT_PLAN.md` に追記する。スコープ外要件は発明しない。

**Step 5: 設計サマリーとレビュー指摘の提示（承認ゲート）**
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🏗️ 設計サマリー
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
テストケース概要:
  ...
  - [DoD2]: [確認するケース]
レビュー指摘（敵対的レビューの結果）:
  🔴 [反映済みの重大指摘] / 🟡 [DRAFT へ回した指摘] / ⚪ [対応不要と判断した指摘]
リスク   : [あれば]
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
▶ この設計（レビュー反映済み）で進めてよいですか？
```

**Step 6: 設計ドキュメントのコミット（承認後）**
- 承認ゲート通過後、設計フェーズで作成・更新したドキュメントをフェーズ1で作成した作業用ブランチにコミットする
- コミット対象：
  - `tasks/T-XXX_<機能名>.md`（設計メモ・Todoリスト・テスト設計を記入したもの）
...
- 設計レビュー（Step 4）はゲート提示の前に行い、指摘を反映・分類してから提示する
- 設計ドキュメントのコミット（Step 6）は承認を得てから実施する
...
- 1ステップ完了ごとに `tasks/T-XXX_<機能名>.md` のTodoリストのチェックボックスを更新し、進捗を報告する：
  ...
  🔄 Step 2 実装中...
  ```
- コードレビューは本フェーズでは行わず、**フェーズ4出口で一括**で実施する（「レビュー工程の併用」参照）。実装↔テストの反復はゲート無しのまま回す。
...
- 全テストケース合格＋リグレッションOKになったら、Step 4 のコードレビューを経てフェーズ5 フィードバックへ進む
...
**Step 4: コードレビュー（フェーズ3+4一括・敵対的スタンス）**
- 全テストケース合格後、フェーズ5へ進む前に、`requesting-code-review` スキルでサブエージェントによるコードレビューを1回実施する（フェーズ3の実装とフェーズ4のテストをまとめて対象にする。「レビュー工程の併用」に従う）。
- レビューは敵対的スタンスで行う：正確性バグ・異常系・セキュリティ・スコープ外の副作用・規約逸脱・「DoD の字面は満たすが趣旨は未達」を能動的に探す。
- 返ってきた指摘は `receiving-code-review` スキルで技術的に吟味する（盲従も全否定もしない）。
- 指摘は既存FB分類（🔴即修正／🔵テスト漏れ／🟡次サイクル／⚪対応不要）で振り分け、フェーズ5 フィードバックの入力として渡す。🔴/🔵 が残る場合は実装↔テストの反復（ゲート無し）で解消してから進む。
- `verification-before-completion` スキルに従い、「テスト合格」を主張・報告する前に、実際の実行結果で裏取りする。
- 本フェーズではコミットしない（コミットはフェーズ5の最終判断後）。

### 完了条件
- [ ] すべての完了条件がパスしている（またはNG内容・断念理由を報告済み）
- [ ] リグレッションがない（または報告済み）
- [ ] テスト結果を `tasks/T-XXX_<機能名>.md` に記録した
- [ ] コードレビュー（フェーズ3+4一括・敵対的スタンス）を実施し、指摘をFB分類で整理した
...
**Step 6.5: 全ドキュメントの整合性レビュー（コミット前）**
- Step 6 で全ドキュメントを更新したら、コミット（Step 7）の前に、サブエージェントで全ドキュメントの相互整合を点検する（整合性レビュー。「レビュー工程の併用」に従う）。
- 観点: `tasks/T-XXX_<機能名>.md`・`docs/OVERALL_PLAN.md`・`docs/ARCHITECTURE.md`・`docs/DECISIONS.md` 等が相互に矛盾しないか、FB分類が妥当か。
- コードは再レビューしない（フェーズ4出口で完了済み）。指摘は既存FB分類で扱う。
...
- 上記のマージ記録・完了移動を `docs/CODING_STANDARDS.md` に従い `docs:` でコミットする（マージ起因のドキュメント更新を未コミットで残さない）：
  ...
  ```
- マージ後、`verification-before-completion` スキルに従い検証する（独立レビューは置かない）：main がグリーンである・コンフリクト痕が残っていない・意図した変更が反映されている・マージ記録／完了移動が正しい。問題があれば修正してから完了とする。
...
- 昇格候補を提示する前に、候補の妥当性・`docs/OVERALL_PLAN.md` との整合を点検する（「レビュー工程の併用」に従い、軽量レビューでよい）。既存の `brainstorming` 併用はそのまま。
